...
def plot_arm_vs_mema_fp32_tput(fname = 'arm_vs_mema_fp32_tput'):
	...
	plt.title('(a) FP32 Computation Throughput of ARM vs MEMA', fontsize = 14)
...
def plot_arm_vs_mema_q15_tput(fname = 'arm_vs_mema_q15_tput'):
	...
	tput_mema = [float(2*(i**3)) / float(df1[(df1['algo'] == 'outer_q15_4x2') \
		& (df1['M'] == i)]['time'].values[0]) for i in N1]
	fig = plt.figure(figsize = (6,4))
	plt.title('(c) Q15 Computation Throughput of ARM vs MEMA', fontsize = 14)
...
def plot_arm_vs_mema_energy(fname = 'arm_vs_mema_energy'):
	...
	out_q15 = compute_energy_peaks('power_outer_q15.csv')[:-1]
	...
	#
	fig = plt.figure(figsize = (6,4))
	plt.title('(b) FP32 Energy Usage During MM in ARM vs MEMA', fontsize = 14)
	plt.plot(fp32_ops, in_fp32, 'b', label = 'arm_inner_f32', marker = markers[1], color = colors[1])
	plt.plot(fp32_ops, out_fp32, 'b', label = 'mema_outer_fp32', marker = markers[4], color = colors[4])
	plt.legend(loc = "lower right", prop={'size': 10})
	...
	#
	fig = plt.figure(figsize = (6,4))
	plt.title('(d) Q15 Energy Usage During MM in ARM vs MEMA', fontsize = 14)
	plt.plot(q15_ops, in_q15, 'b', label = 'arm_inner_q15', marker = markers[1], color = colors[1])
	plt.plot(q15_ops, out_q15, 'b', label = 'mema_outer_q15', marker = markers[4], color = colors[4])
	plt.legend(loc = "lower right", prop={'size': 10})
